...
import styles from "./FreeLancingScreen.module.css";
    ...
    <div className={styles.mainContainer}>
      <div className={styles.titleContainer}>
        ...
        <h5 style={{ marginLeft: 20, marginTop: -14, paddingRight: 20 }}>
          Hire a professional for your personal business
        </h5>
        <SearchBarJobs />
      </div>
      <div className={styles.workersContainer}>
        {dummyWorkers.map(item => <WorkerCard img={item.img} name={item.name} occupation={item.occupation} desc={item.desc} date={item.date} location={item.location} />)}
        {dummyWorkers.map(item => <WorkerCard img={item.img} name={item.name} occupation={item.occupation} desc={item.desc} date={item.date} location={item.location} />)}
        {dummyWorkers.map(item => <WorkerCard img={item.img} name={item.name} occupation={item.occupation} desc={item.desc} date={item.date} location={item.location} />)}
      </div>
    </div>
...
export default FreeLancingScreen;
